...
void Framework::Init(int width, int height, int fps, int shipSize, int missileSize) {
    ...
    if (TTF_Init() != 0) {
        ...
        // Handle the error
        // Exit your application or return an error code
    }
...
void Framework::RenderText(const std::string& text, int x, int y, TTF_Font* font, SDL_Color color) {
                    ...
                    if (textSurface == nullptr) {
                        cerr << "Error: Text rendering failed." << endl;
                        return;
                    }

                    SDL_Texture* textTexture = SDL_CreateTextureFromSurface(renderer, textSurface);
                    if (textTexture == nullptr) {
                        cerr << "Error: Texture creation failed." << endl;
...
SDL_Texture * Framework::GetTexture(string imgName) {
    string imgPath = "assets/images" + pathSep + imgName;
